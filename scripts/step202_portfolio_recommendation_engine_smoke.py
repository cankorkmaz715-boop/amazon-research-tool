#!/usr/bin/env python3
"""
Step 202 smoke test: Portfolio recommendation engine.
Validates recommendation generation, classification output, empty-state resilience, payload stability, isolation, fallback.
"""
import os
import sys
...
ENTRY_KEYS = {"item_type", "item_key", "item_label", "recommendation_status", "priority_level", "rationale", "supporting_signals", "recommended_action", "risk_notes"}
RECOMMEND_STATUSES = ("add_to_portfolio", "continue_monitoring", "archive_or_deprioritize")


def main() -> None:
    from amazon_research.portfolio_recommendations import (
    ...
    # --- Recommendation generation: returns dict with required top-level keys
    try:
        out = generate_workspace_portfolio_recommendations(99991)
    ...
    try:
        out = generate_workspace_portfolio_recommendations(1)
        for lst in (out.get("add_recommendations") or [], out.get("monitor_recommendations") or [], out.get("archive_recommendations") or []):
            for entry in lst:
    ...
    try:
        empty_out = generate_workspace_portfolio_recommendations(None)
        if not isinstance(empty_out, dict) or not REQUIRED_TOP_KEYS.issubset(empty_out.keys()):
            empty_ok = False
        if (empty_out.get("add_recommendations") or []) != []:
            empty_ok = False
    except Exception as e:
        empty_ok = False
        print(f"empty-state resilience error: {e}")

    # --- Payload stability: recommendation_summary, rationale_summary, confidence_indicators are dicts
    try:
        out = generate_workspace_portfolio_recommendations(99992)
        if not isinstance(out.get("recommendation_summary"), dict):
            payload_ok = False
        if not isinstance(out.get("rationale_summary"), dict):
            payload_ok = False
        if not isinstance(out.get("top_portfolio_actions"), list):
            payload_ok = False
    ...
    try:
        if not require_workspace_context(1, "portfolio_recommendations"):
            isolation_ok = False
        out = generate_workspace_portfolio_recommendations(1)
    ...
    # --- Fallback behavior: when strategy fails, output has signal_fallback or stable shape; no crash
    try:
        out = generate_workspace_portfolio_recommendations(99993)
        if not isinstance(out, dict):
            fallback_ok = False
        if "add_recommendations" not in out:
            fallback_ok = False
    except Exception as e:
        fallback_ok = False
        print(f"fallback behavior error: {e}")

    print("portfolio recommendation engine OK" if (gen_ok and classification_ok and empty_ok and payload_ok and isolation_ok and fallback_ok) else "portfolio recommendation engine FAIL")
